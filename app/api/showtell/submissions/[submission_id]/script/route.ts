...
import { Script, WritingSubmission } from "@/app/types/showtell";
import { 
  getStyleGuide, 
 ...
 * 
 * AI Script Coach가 생성한 발표 대본 저장
 * - 스타일 가이드에 따라 레벨별 문장 수/길이 제어
 * - Writing Ladder Level 0: 더 엄격한 제약
 * - Show & Tell: 더 유연한 제약
 */
export async function POST(
  ...
  try {
    ...
    const course_type = course_id.startsWith("writing_ladder") ? "writing_ladder" : "show_tell";
    ...
    if (!styleGuide) {
      console.log(`⚠️ 스타일 가이드 없음 (${grade_band}/${cefr_level}/${course_type}), 기본값 사용`);
      styleGuide = getDefaultStyleGuide(course_type);
    }
    ...
    const structure = {
      ...
      conclusion: script_30s_sentences[script_30s_sentences.length - 1] ? script_30s_sentences[script_30s_sentences.length - 1].trim() + "." : "Thank you for listening.",
    };
...
/**
 * GET /api/showtell/submissions/{submission_id}/script
 * 
...
export async function GET(
  ...
  try {
    ...
    // Script 찾기 (submission_id로)
    // Note: 실제로는 query로 찾아야 하지만, 여기서는 간단히 week_progress에서 가져옴
    const submission = submissionDoc.data() as WritingSubmission;
    ...
    const script_id = progressData.day2_script_id;

    if (!script_id) {
      return NextResponse.json(
        { success: false, error: "Script not yet generated" },
        { status: 404 }
      );
    }
...
function getMockScript(
  ...
  return {
    ...
    keywords,
    simplified_sentences: [
      "extremely excited → very excited",
...
JSON 형식으로 반환: {"sentences": ["문장1", "문장2", "문장3"]}`
    ...
    if (!Array.isArray(selectedSentences) || selectedSentences.length !== 3) {
      console.warn("⚠️ GPT가 3개가 아닌 문장 반환, fallback 사용");
      return allSentences.slice(0, 3);
    }

    console.log("✅ GPT-3.5가 선택한 3문장:", selectedSentences);
    return selectedSentences;

  } catch (error: unknown) {
    const err = error as Error;
    console.error("❌ AI 문장 선택 오류:", err);
    console.warn("⚠️ Fallback: 기본 방식으로 앞 3문장 선택");
