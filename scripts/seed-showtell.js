/**
 * Show & Tell 12주 코스 시드 스크립트
 * 
 * Firestore에 콘텐츠 데이터를 업로드합니다:
 * - topics (12개)
 * - judge_question_sets (12개)
 * - content_packs (12개)
 * 
 * 사용법:
 * node scripts/seed-showtell.js
 */
...
function loadJSON(filename) {
  const filePath = path.join(__dirname, "..", "seed", "showtell", filename);
...
// 메인 실행 함수
async function main() {
  console.log("🚀 Show & Tell 12주 코스 시드 시작\n");
  console.log("=" .repeat(50));
  
  try {
    // 1. Topics 업로드
    const topics = loadJSON("topics.json");
    await uploadCollection("showtell_topics", topics, "topic_id");
    
    // 2. Judge Question Sets 업로드
    const questionSets = loadJSON("judge_question_sets.json");
    await uploadCollection("showtell_judge_questions", questionSets, "question_set_id");
    
    // 3. Content Packs 업로드
    const contentPacks = loadJSON("content_packs.json");
    await uploadCollection("showtell_content_packs", contentPacks, "content_pack_id");
